...
class PeriodePerkuliahanController extends Controller
{
    public function index(Request $req)
    {
        ...
        if ($req->has('semester') || $req->has('prodi')) {
            $periode_kuliah = $data->select('id_semester', 'pd_feeder_list_periode_perkuliahan.id_prodi', 'pd_feeder_list_periode_perkuliahan.nama_program_studi', 'nama_semester', 'jumlah_target_mahasiswa_baru', 'tanggal_awal_perkuliahan','tanggal_akhir_perkuliahan','pd_feeder_program_studi.status')
            ...
            $periode_kuliah = $data->select('id_semester', 'pd_feeder_list_periode_perkuliahan.id_prodi', 'pd_feeder_list_periode_perkuliahan.nama_program_studi', 'nama_semester', 'jumlah_target_mahasiswa_baru', 'tanggal_awal_perkuliahan','tanggal_akhir_perkuliahan','pd_feeder_program_studi.status')->where('pd_feeder_list_periode_perkuliahan.nama_semester', $semester_aktif[0]['nama_semester'])
            ->when($req->has('p') || $req->has('keyword') || $req->has('prodi') || $req->has('semester'), function($q) use($req){
                if ($req->keyword != '') {
                    $q->where('pd_feeder_list_periode_perkuliahan.nama_program_studi', 'like', '%'.$req->keyword.'%')
                    ->orWhere('pd_feeder_list_periode_perkuliahan.nama_semester', 'like', '%'.$req->keyword.'%');
        ...
        return view('backend.univ.pelengkap.periode-perkuliahan.index', compact('periode_kuliah','prodi','semester','semester_aktif','val','paginate', 'valPaginate'));
    }
    ...
    public function detail($prodi,$semester)
    {
        ...
        return view('backend.univ.pelengkap.periode-perkuliahan.detail', compact('detail_periode_kuliah'));
    }
}
