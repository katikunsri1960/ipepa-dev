...
class DosenController extends Controller
{
    ...
    public function penugasan($id)
    {
        ...
        $dosen = DetailPenugasanDosen::where('id_dosen', $id)->select('nama_tahun_ajaran', 'nama_program_studi', 'no_surat_tugas', 'tanggal_surat_tugas', 'mulai_surat_tugas')
        ->addSelect(DB::raw('(SELECT CONCAT("kode_perguruan_tinggi","-","nama_perguruan_tinggi") FROM pd_feeder_perguruan_tinggi WHERE id_perguruan_tinggi = pd_feeder_detail_penugasan_dosen.id_perguruan_tinggi LIMIT 1) AS perguruan_tinggi'))->first();

        return view('backend.univ.dosen.penugasan-dosen', compact('dosen'));
    }
}
